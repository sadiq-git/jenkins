pipeline {
  ...
  environment {
    // Option A: talk to the planner via the host-published port
  ...
  stages {
    ...
    stage('Checkout') {
      steps {
        script {
          deleteDir() // ensure no half-baked workspace
        }
    ...
    stage('AI Planning (Gemini)') {
      steps {
        script {
          // Add host.docker.internal for Linux too; harmless on Docker Desktop
          ...
          docker.image('node-ci:20-bookworm-slim').inside(dockerNetOpts) {
            sh label: 'Request plan from AI', script: '''
              ...
              # 1) quick health check (do not -f; we want status even if 404)
